    ...
    limits = {
        "liquid": {
            "water": {
                "suction": {
                    "saturation": {
                        "all": {"velocity_min": 0.61, "velocity_max": 1.83, "dp_min": None, "dp_max": 0.12}
                    },
                    "normal": {
                        "all": {"velocity_min": 1.22, "velocity_max": 2.13, "dp_min": None, "dp_max": None}
                    }
                },
                "discharge": {
                    "normal": {
                        "less than 1": {"velocity_min": 0.61, "velocity_max": 0.91, "dp_min": None, "dp_max": None},
                        "less than 2": {"velocity_min": 0.91, "velocity_max": 1.37, "dp_min": None, "dp_max": None},
                        "less than 4": {"velocity_min": 1.52, "velocity_max": 2.13, "dp_min": None, "dp_max": None},
                        "less than 6": {"velocity_min": 2.13, "velocity_max": 2.74, "dp_min": None, "dp_max": None},
                        "less than 8": {"velocity_min": 2.74, "velocity_max": 3.05, "dp_min": None, "dp_max": None},
                        "less than 10": {"velocity_min": 3.05, "velocity_max": 3.66, "dp_min": None, "dp_max": None},
                        "less than 12": {"velocity_min": 3.05, "velocity_max": 4.27, "dp_min": None, "dp_max": None},
                        "less than 16": {"velocity_min": 3.05, "velocity_max": 4.57, "dp_min": None, "dp_max": None},
                        "above 16": {"velocity_min": 3.05, "velocity_max": 4.88, "dp_min": None, "dp_max": None}
                    }
                }
            },
            "HC": {
                "suction": {
                    "saturation": {
                        "all": {"velocity_min": 0.61, "velocity_max": 1.83, "dp_min": None, "dp_max": None}
                    },
                    "normal": {
                        "all": {"velocity_min": 1.22, "velocity_max": 2.44, "dp_min": None, "dp_max": None}
                    }
                },
                "discharge": {
                    "normal": {
                        "all": {"velocity_min": 1.5, "velocity_max": 4.5, "dp_min": None, "dp_max": 0.23}
                    }
                }
            }
        },
        "vapor": {
            "steam": {
                "all": {
                    "normal": {
                        "all": {"velocity_min": None, "velocity_max": 60.96, "dp_min": None, "dp_max": None}
                    }
                }
            },
            "gas": {
                "less than 3.5": {
                    "normal": {
                        "all": {"velocity_min": None, "velocity_max": None, "dp_min": None, "dp_max": 0.06}
                    }
                },
                "less than 10.5": {
                    "normal": {
                        "all": {"velocity_min": None, "velocity_max": None, "dp_min": None, "dp_max": 0.11}
                    }
                },
                "less than 21.1": {
                    "normal": {
                        "all": {"velocity_min": None, "velocity_max": None, "dp_min": None, "dp_max": 0.23}
                    }
                },
                "above 21.1": {
                    "normal": {
                        "all": {"velocity_min": None, "velocity_max": None, "dp_min": None, "dp_max": 0.35}
                    }
                }
            }
        },
        "2 phase": {
            "all": {
                "all": {
                    "all": {
                        "all": {"velocity_min": 10.67, "velocity_max": 22.86, "dp_min": None, "dp_max": None}
                    }
                }
            }
        }
        ...
        for j, route in enumerate(routes):
            for i, object in enumerate(route) :
                if object['type'] == "liquid_line" : 
                    ...
                    # route[i]['limitation']['phase'] ='liquid'
                    # route[i]['limitation']['fluid'] = 'water' if route[i]['density'] > 900 else 'HC'
                    # pump_indices = [idx for idx, obj in enumerate(route) if obj['type'] in ['fixed_pump', 'variable_pump']]
                    # for i in range(len(route)):
                    #     if pump_indices:
                    #         # 위치 판단
                    #         route[i]['limitation']['location'] = ('suction' if any(i < pump_idx for pump_idx in pump_indices) else 'discharge')
                    #         # 가장 가까운 펌프의 vapor_pressure 기준으로 상태 판단
                    #         nearest_pump_idx = min(pump_indices, key=lambda x: abs(x - i))
                    #         vapor_pressure = route[nearest_pump_idx].get('vapor_pressure', 0)
                    #         route[i]['limitation']['state'] = ('saturation' if vapor_pressure > 1 else 'normal')
                    #     else:
                    #         # 펌프가 없을 경우 기본값 설정
                    #         route[i]['limitation']['location'] = 'discharge'
                    #         route[i]['limitation']['state'] = 'normal'
            
                    # # route[i]['limitation']['state'] = 'normal' # 추후 온도 넣는 항목을 만들어 if문 추가 예정
                    # if route[i]['limitation']['location'] == "suction" and route[i]['limitation']['fluid'] == 'HC' :
                    #     route[i]['limitation']['pipesize'] = 'less than 8' if route[i]['pipe_size_ID'] < 8.2 else 'above 8'
                    # if route[i]['limitation']['location'] == "suction" and route[i]['limitation']['fluid'] == 'water' :    
                    #     route[i]['limitation']['pipesize'] = 'less than 8' if route[i]['pipe_size_ID'] < 8.2 else 'above 8'
                    # if route[i]['limitation']['location'] == "discharge" and route[i]['limitation']['fluid'] == 'HC' :
                    #     route[i]['limitation']['pipesize'] = 'all'
                    # if route[i]['limitation']['location'] == "discharge" and route[i]['limitation']['fluid'] == 'water' :    
                    #     pipe_size = route[i]['pipe_size_ID']
                    #     if pipe_size < 1.1:
                    #         route[i]['limitation']['pipesize'] = 'less than 1'
                    #     elif pipe_size < 2.2:
                    #         route[i]['limitation']['pipesize'] = 'less than 2'
                    #     elif pipe_size < 4.4:
                    #         route[i]['limitation']['pipesize'] = 'less than 4'
                    #     elif pipe_size < 6.4:
                    #         route[i]['limitation']['pipesize'] = 'less than 6'
                    #     elif pipe_size < 8.2:
                    #         route[i]['limitation']['pipesize'] = 'less than 8'
                    #     elif pipe_size < 10.25:
                    #         route[i]['limitation']['pipesize'] = 'less than 10'
                    #     elif pipe_size < 12.26:
                    #         route[i]['limitation']['pipesize'] = 'less than 12'
                    #     elif pipe_size < 16:
                    #         route[i]['limitation']['pipesize'] = 'less than 16'
                    #     else:
                    #         route[i]['limitation']['pipesize'] = 'above 16'
                    ...
                    pipe_size = route[i]['pipe_size_ID']
                    if location == "suction":
                        pipesize = 'less than 8' if pipe_size < 8.2 else 'above 8'
                    elif location == "discharge" and fluid == 'HC':
                        pipesize = 'all'
                    elif location == "discharge" and fluid == 'water':
                        if pipe_size < 1.1:
                ...
                if object['type'] == "vapor_line" :
                    ...
                    # route[i]['limitation']['phase'] ='vapor'
                    # route[i]['limitation']['fluid'] = 'steam' if abs(route[i]['mw'] - 18.01528) < 0.025 else 'gas'
                    # route[i]['limitation']['pipesize'] ='all'
                    # route[i]['limitation']['state'] = 'normal'
                    # if route[i]['limitation']['fluid'] == "gas":              
                    #     P_in = route[i]['P_in']
                    #     if P_in < 3.5:
                    #         route[i]['limitation']['location'] = 'less than 3.5'
                    #     elif P_in < 10.5:
                    #         route[i]['limitation']['location'] = 'less than 10.5'
                    #     elif P_in < 21.1:
                    #         route[i]['limitation']['location'] = 'less than 21.1'
                    #     else:
                    #         route[i]['limitation']['location'] = 'above 21.1'
                    # if route[i]['limitation']['fluid'] == "steam":  
                    #     route[i]['limitation']['location'] = 'all'
                    phase = 'vapor'
...
#                                     style += ';strokeWidth=2'
#                                 obj['style'] = style
#                             # obj['strokeWidth'] = 2  # 이렇게 하면 XML에는 반영 안 됨!
#                         only_lines.append(obj)
#             # 첫 번째 route 처리 완료
#             if not first_route_processed:
