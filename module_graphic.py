...
def validate_object_data(object):
    ...
    if 'i1_static_loss' in object.attrib:  #object.find('.//mxCell[@edge="1"]'): # check input of user line
        if not is_convertible_to_number(object.attrib.get('i1_static_loss')) :
            error_message = "line no. " + str(line_number) + " {error: i1_static_loss = " + object.attrib.get('i1_static_loss') +" }"
            input_error_message.append (error_message) 
        if not is_convertible_to_number(object.attrib.get('i2_pipe_loss')) :
            error_message = "line no. " + str(line_number) + " {error: i2_pipe_loss = " + object.attrib.get('i2_pipe_loss') +" }"
            input_error_message.append (error_message) 
        line_number +=1
