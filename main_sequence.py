...
def calculation_sequence (file_path, running_status):
    ...
    if calculation_sheet is not None:
        ...
        for obj in calculation_sheet.findall(".//object"):
            if obj.get("hydro_version") and obj.get("pressure_selection") and obj.get("running_status"):
                ...
                config.pressure_selection = obj.get("pressure_selection")
    ...
    # print (sorted_line_data)

    # pressure_selection 값에 따라 단위 설정
    # pressure_unit = 'bar' if config.pressure_selection == 'bar' else 'kg/cm2'
    pressure_unit = 'bar' if config.pressure_selection == 'bar' else 'MPa' if config.pressure_selection == 'MPa' else 'kg/cm2'
    ...
    index = [
        ...
        f'static loss [{pressure_unit}]',
        f'pipe loss [{pressure_unit}]',
    ...
    # df = pd.DataFrame(sorted_line_data, index + ['criteria'])  # criteria 포함
    # print(sorted_line_data)
    (
        root_elem,                               # 수정된 root element
        CalculationResult_layer_id,              # 'Calculation Result' 레이어 ID
        top_left_position_stream,                # stream 좌상단 기준 좌표
        top_left_position_control_valve,         # 제어밸브 좌상단 기준 좌표
        top_left_position_pump_context_box,      # pump context box 좌표
        top_left_position_deviation_context_box, # deviation context box 좌표
    ) = module_output_drawio.clear_calculation_result_contents_only(root)

    # print(top_left_position)
    if top_left_position_stream: # 사용자가 이동하면서 지정한 위치
        start_x, start_y = top_left_position_stream
        start_y = start_y -15
    else:
        start_x, start_y = 30, 900
    module_output_drawio.add_StreamData_to_layer(df, root_elem, CalculationResult_layer_id, start_x=start_x, start_y=start_y)
    ...
    if (sorted_control_valves) :  # control valve가 없는 경우도 많다.
        index_cv = ['inlet P', 'outlet P', f'dP [{pressure_unit}]' ]       
        df_cv = pd.DataFrame (sorted_control_valves, index_cv)
        # print(top_left_position_control_valve)
        if top_left_position_control_valve: # 사용자가 이동하면서 지정한 위치
            start_x, start_y = top_left_position_control_valve
            start_y = start_y -15
        else:
            start_x, start_y = 958, 20        
        module_output_drawio.add_CVData_to_layer(df_cv, root_elem, CalculationResult_layer_id, start_x=start_x, start_y=start_y, index_col_width=55, col_width=45, cell_height=15, font_size=9, max_cols=3, row_spacing=2)
    ...
    if top_left_position_pump_context_box: # 사용자가 이동하면서 지정한 위치
        start_x, start_y = top_left_position_pump_context_box
        # start_y = start_y -15
    else:
        start_x, start_y = 960, 500      
    module_output_drawio.add_string_content_to_layer (only_contents, root_elem, CalculationResult_layer_id, start_x=start_x, start_y=start_y, col_width=100, cell_height=100, font_size=10, box_name="pump_content_box")
    
    # ✅ deviation 처리 
    only_deviation = module_hyd.extract_deviation (final_grouped_routes)
    # print (top_left_position_deviation_context_box)
    if top_left_position_deviation_context_box: # 사용자가 이동하면서 지정한 위치
        start_x, start_y = top_left_position_deviation_context_box
        # start_y = start_y -15
    else:
        start_x, start_y = 15, 15          
    module_output_drawio.add_string_content_to_layer (only_deviation, root_elem, CalculationResult_layer_id, start_x=start_x, start_y=start_y, col_width=400, cell_height=30, font_size=10, box_name="deviation_content_box")
    ...
    def generate_watermark_text():
        ...
        # config.user_name = usb_time_auth.get_current_volume_name()
        watermark_text = f"HYDRO CONFIDENTIAL : {ip_address} | {hostname}"
        return watermark_text

    # ✅ 교육용 워터마크 추가    
    module_output_drawio.add_watermark_to_layer (root_elem, CalculationResult_layer_id)
