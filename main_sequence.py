...
def calculation_sequence (file_path, running_status):
    ...
    # ✅ Loop 안의 data input 이름을 계산에 편한 보편적 이름으로 바꾸고 문자를 숫자로 변경
    ...
    root_elem, stream_layer_id = module_output_drawio.clear_stream_result_contents_only(root)
    # print(df)
    module_output_drawio.add_dataframe_to_layer(df, root_elem, stream_layer_id,start_x=30, start_y=850)
    # self.text_editor.append(df.to_string(index=True)) # 사용자 뷰에 stream table 보여주기  
    module_output_drawio.tree_write_lines (file_path, root, only_lines)
    # print(df)
    ...
    index_cv = ['inlet P', 'outlet P', '\u0394P [kg/cm2]' ]       
    df_cv = pd.DataFrame (sorted_control_valves, index_cv)
    # print(df_cv)
    module_output_drawio.add_dataframe_to_layer(df_cv, root_elem, stream_layer_id, start_x=960, start_y=20, index_col_width=60, col_width=50, cell_height=15, font_size=9, max_cols=2, row_spacing=2)
    ...
    module_output_drawio.add_string_content_to_drawio (only_contents, root_elem, stream_layer_id, start_x=960, start_y=500, col_width=100, cell_height=100, font_size=10)
    
    # ✅ deviation 처리 
    only_deviation = module_hyd.extract_deviation (final_grouped_routes)
    # print (only_deviation)
    module_output_drawio.add_string_content_to_drawio (only_deviation, root_elem, stream_layer_id, start_x=15, start_y=15, col_width=400, cell_height=30, font_size=10)
