...
#                 # phase는 첫 번째 행에서 열별로 가져옴
#                 if df.shape[0] > 0 and col_offset + col_index < len(df.columns):
#                     phase = str(df.iloc[0, col_offset + col_index]).strip().lower()
#                     limits = {
#                         "liquid": {"velocity": 5.0, "dp": 0.5},
#                         "vapor": {"velocity": 50.0, "dp": 5.0},
#                         "2 phase": {"velocity": 25.0, "dp": 1.0}
#                     }
#                     velocity_limit = limits.get(phase, {}).get("velocity")
#                     dp_limit = limits.get(phase, {}).get("dp")
#                 else:
#                     velocity_limit = None
#                     dp_limit = None
...
#                 # 조건부 스타일 적용
#                 try:
#                     # print(phase, idx, value)
#                     if idx == "velocity [m/sec]" and velocity_limit is not None:
#                         if float(value) >= velocity_limit:
#                             value = f"<font color='#ff0000'>{value}</font>"                            
#                             style += "fontColor=red;"
#                     elif "ΔP" in idx and dp_limit is not None:
#                         if float(value) >= dp_limit:
#                             value = f"<font color='#ff0000'>{value}</font>"
#                             style += "fontColor=red;"
#                 except Exception:
#                     pass  # 숫자 변환 실패 시 무시
...
def add_dataframe_to_layer(df, root_elem, parent_id, start_x=30, start_y=900,
    ...
    def add_cells(df, root_elem, parent_id, start_x, start_y, index_col_width,
        ...
        #     print(df.iloc[row_index, -1], type(df.iloc[row_index, -1]))  # 마지막 컬럼 (criteria dict)

        # for idx, row in df.iterrows():
            # criteria = row.iloc[-1]  # 마지막 컬럼
            # print(criteria)          # dict 타입 출력
            # print(type(criteria))    # <class 'dict'>

        # criteria_list = []
        # for idx, row in df.iterrows():
            # criteria = row.iloc[-1]
            # if isinstance(criteria, dict):
            #     min_vel = criteria.get('min_vel')
            #     max_vel = criteria.get('max_vel')
            #     min_dp = criteria.get('min_dp')
            #     max_dp = criteria.get('max_dp')
            ...
            for col_index, value in enumerate(row[col_offset:col_offset + max_cols]):
                ...
                try:
                    ...
                    if idx == "velocity [m/sec]" : #and velocity_min is not None and velocity_max is not None:
                        print(value, velocity_max)
                        if float(value) < velocity_min or float(value) > velocity_max:
                            value = f"<font color='#ff0000'>{value}</font>"
                            style += "fontColor=red;"
                    elif "ΔP" in idx and dp_min is not None and dp_max is not None:
                        if float(value) < dp_min or float(value) > dp_max:
                            value = f"<font color='#ff0000'>{value}</font>"
                            style += "fontColor=red;"
                except Exception:
                    pass
