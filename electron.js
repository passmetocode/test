	...
	try
	{
		...
		case 'showOpenDialog':
			...
			ret = await showOpenDialog(args.defaultPath, args.filters, args.properties);
                ...
                //     type: 'info',
                //     title: '파일 ㅌㅌㅌ 경로 알림',
                //     message: 'open-file-path로 전송된 파일 경로: \n' + ret.filePaths.join("\n")
                // });
...
// 	const proc = spawn(hydroPath, ['--from-drawio', 'secure-token-123']);
...
// 	const scriptPath = path.resolve(__dirname, 'C:\\Dev\\Hydrocal\\Hydrocal.py');
//     //const scriptPath = path.join(process.cwd(), 'Hydrocal.exe');
...
//     const scriptPath = path.resolve(__dirname, 'C:\\Dev\\Hydrocal\\Hydrocal.py');
//     const args = [scriptPath, '--usb-check'];
...
//         // 예: renderer로 넘기기 위한 전체 사용자 정보 반환
//         resolve(result);

//         // 또는 조건 분기 후 처리할 수도 있음
//         /*
//         if (result.user === 'normal_user') {
//           resolve({ status: 'ok', info: result });
//         } else {
//           resolve({ status: 'block', reason: result.user });
//         }
//         */
//       } catch (e) {
//         reject(`❌ JSON 파싱 오류: ${e}`);
