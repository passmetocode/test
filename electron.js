...
function createWindow (opt = {})
{
	...
	let options = Object.assign( // hydro_yangs 
	{
		backgroundColor: '#FFF',
		width: parseInt(lastWinSize[0]),
		height: parseInt(lastWinSize[1]),
		icon: `${codeDir}/js/hydrologo.png`,
		webviewTag: false,
		webSecurity: true,
		webPreferences: {
...
// 설정 저장 경로를 %APPDATA%\hydro 로 강제 설정
...
app.whenReady().then(() =>
{
	...
	ipcMain.on('newfile', (e, arg) =>
	{
		...
		// 실행파일 기준 상위 폴더로 이동 후, HydroData 폴더 생성
		const appBasePath = isDev
			? "P:\\Hydro"
			: path.resolve(path.dirname(process.execPath), '..', 'template');
...
if (!gotTheLock) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        function loadFinished(e) {
            if (e != null && !validateSender(e.senderFrame)) return;

            loadEvtCount++;

            if (loadEvtCount === 2) {
                // 외부 인자에서 파일 경로 추출
                const potFile = commandLine.find(arg => arg.endsWith('.hydro'));

                if (potFile && fs.existsSync(potFile)) {
...
if (!program.args || program.args.length === 0) {
    // 인자가 없으면 default.hydro 사용
    program.args = [defaultPath];
}

// 인자가 있을 경우 default.hydro는 무시
if (program.args && program.args.length > 0) {
...
{
	let fh;
	let modifiedData = data;  // <- 함수 최상단에서 선언 (전역처럼 쓰이도록)

	try
	{
		// hydro_yangs: mxfile → hydrofile 변환
		if (typeof modifiedData === 'string' && modifiedData.includes('<mxfile'))
		{
			modifiedData = modifiedData.replace('<mxfile', '<hydrofile').replace('</mxfile', '</hydrofile');
		}

		fh = await fsProm.open(fileObject.path, O_SYNC | O_CREAT | O_WRONLY | O_TRUNC);
		await fsProm.writeFile(fh, modifiedData, writeEnc);
		await fh.sync(); 
	...
	// 🔧 실제로 쓴 modifiedData 기준으로 비교
	if (modifiedData != writtenData)
	{
		retryCount++;
	...
	{
		if (backupCreated && fs.existsSync(oldBkpPath))
		{
...
async function readFile(filename, encoding) // hydro_yangs
{
	let data = await fsProm.readFile(filename, encoding);

	// hydro_yangs: <hxfile> → <mxfile> 변환
	if (typeof data === 'string' && data.includes('<hydrofile'))
	{
		data = data.replace('<hydrofile', '<mxfile').replace('</hydrofile', '</mxfile');
	}
...
// 예시로 글로벌 변수, 실환경에서는 알맞게 수정!

global.currentOpenedFilePath = null;

function openExternal(urlOrPath) {
    const isDev = !app.isPackaged;
    const filePath = global.currentOpenedFilePath;
    const baseFolder = filePath
        ? path.dirname(filePath)
        : (isDev
            ? "P:\\Hydro"
            : path.resolve(path.join(process.resourcesPath, 'data')));

    try {
        // 1. http/https 링크는 그대로 외부 실행
        if (/^https?:\/\//i.test(urlOrPath)) {
            shell.openExternal(encodeURI(urlOrPath));
            return true;
        }

        // 2. file:/// 경로 처리 (webapp 기준 상대 경로)
        if (/^file:\/\/\//i.test(urlOrPath)) {
            let rawPath = urlOrPath.replace(/^file:\/\/\//i, '').replace(/\\/g, '/');
            const webappKeyword = '/webapp/';
            let index = rawPath.toLowerCase().indexOf(webappKeyword);
            let relativePath = rawPath;

            if (index >= 0) {
                relativePath = rawPath.substring(index + webappKeyword.length);
            }
            ...
            if (fs.existsSync(targetPath)) {
                shell.openPath(targetPath); // ✅ 한글/공백 대응
                return true;
            } else {
                dialog.showMessageBoxSync({
                    type: 'warning',
                    title: 'No file',
                    message: `Non-existent file path:\n${targetPath}`
                });
                return false;
            }
        }
        ...
        // 3. file:// 경로 처리 (절대 경로)
        if (/^file:\/\//i.test(urlOrPath)) {
            let rawPath = urlOrPath.replace(/^file:(\/\/)?/i, '');
            ...
            if (fs.existsSync(decodedPath)) {
                shell.openPath(decodedPath); // ✅ 한글/공백 대응
                return true;
            } else {
                dialog.showMessageBoxSync({
                    type: 'warning',
...
ipcMain.on("rendererReq", async (event, args) => 
{
	...
	try
	{
		...
		case 'showOpenDialog':
			...
			ret = ret.filePaths;
			if (ret && ret.length > 0) {
				global.currentOpenedFilePath = ret[0];  // 💾 저장!
				//console.log('[DEBUG] currentOpenedFilePath set to:', global.currentOpenedFilePath);
			}
			dialogOpen = false;
			break;
		case 'showSaveDialog':
		...
		case 'readFile':
			...
			if (ret && ret.length > 0) {
				const selectedPath = args.filename || ret[0];
				const ext = path.extname(selectedPath).toLowerCase();

				if (ext === '.hydro') {
					global.currentOpenedFilePath = selectedPath;
					//console.log('[DEBUG] currentOpenedFilePath set to:', global.currentOpenedFilePath);
				} else {
					//console.log('[DEBUG] .hydro 아님 → 이전 값 유지:', global.currentOpenedFilePath);
				}
			}
...
ipcMain.handle('run-hydro', async (event, filePath, currentPageName, userOption) => {
  return new Promise((resolve, reject) => {
    const pythonPath = isPackaged ? path.join(process.resourcesPath, 'Hydrocal', 'Hydrocal.exe') : 'python';
    const scriptPath = isPackaged
      ? pythonPath // EXE 자체가 스크립트
      : path.join(__dirname, 'Hydrocal', 'Hydrocal.py');

    const args = isPackaged
      ? [filePath, currentPageName, userOption]
      : [scriptPath, filePath, currentPageName, userOption];
...
ipcMain.handle('check-usb-key', async () => {
  return new Promise((resolve, reject) => {
    const pythonPath = isPackaged
      ? path.join(process.resourcesPath, 'Hydrocal', 'Hydrocal.exe') // PyInstaller로 만든 exe
      : 'python'; // 개발 중

    const scriptPath = isPackaged
      ? pythonPath // exe 실행이면 scriptPath는 의미 없음
      : path.join(__dirname, 'Hydrocal', 'Hydrocal.py');

    const args = isPackaged
      ? ['--usb-check']                    // exe: 첫 번째 arg부터 전달
      : [scriptPath, '--usb-check'];      // py: script + args
    ...
    proc.on('close', (code) => {
      try {
        ...
        resolve(result); // ✅ JSON 파싱 성공 시 반환
      } catch (e) {
        reject(`❌ JSON 파싱 오류: ${e}\n출력 내용:\n${output}`);
      }
    });
    ...
    proc.on('error', (err) => {
      reject(`❌ USB 체크 실패: ${err}`);
    });
  });
});


ipcMain.on('edit-stream-data', (event, filePath, currentPageName) => {
  const pythonPath = isPackaged
    ? path.join(process.resourcesPath, 'Hydrocal', 'edit_stream_data.exe')
    : 'python';

  const scriptPath = isPackaged
    ? pythonPath
    : path.join(__dirname, 'Hydrocal', 'edit_stream_data.py');

  const args = isPackaged
    ? [filePath, currentPageName]
    : [scriptPath, filePath, currentPageName];

  const proc = spawn(pythonPath, args, { encoding: 'utf8' });

  proc.on('error', (err) => {
    console.error('❌ Failed to run edit_stream_data:', err);
  });

  proc.on('close', (code) => {
    console.log(`✅ edit_stream_data exited with code ${code}`);
  });
});
