...
//			this.addMenuItems(menu, ['-', 'pageSetup', 'print', '-', 'close', '-', 'exit'], parent);
			this.addMenuItems(menu, ['print', '-', 'exit'], parent);
		})));
	};
	...
	App.prototype.chooseFileEntry = async function(fn)
	{
		...
		var paths = await requestSync({
			action: 'showOpenDialog', // hydro_yangs: changed to showOpenDialog
			defaultPath: lastDir || (await requestSync('getDocumentsFolder')),
			filters: [
				{ name: 'Hydro & Symbol File', extensions: ['hydro', 'xml' /*,'drawio',  'png', 'svg', 'html'*/] }, 
        	    // { name: 'User Symbol File', extensions: ['xml'] },
        	    { name: 'All Files', extensions: ['*'] }
			],
			properties: ['openFile']
		});
	...
	EditorUi.prototype.normalizeFilename = function(title, defaultExtension)
	{
		...
		defaultExtension = (defaultExtension != null) ? defaultExtension : 'hydro';


		if (tokens.length == 1 || mxUtils.indexOf(['xml', 'html', 'hydro', 'png', 'svg'], ext) < 0)
		{
		...
		var readData = mxUtils.bind(this, function (data)
		{
			...
			if (isVsdx)
			{
				...
				this.importVisio(data, mxUtils.bind(this, function(xml)
				{
					...
					if (dot >= 0)
					{
						name = name.substring(0, name.lastIndexOf('.')) + '.hydro';
					}
					else
					{
						name = name + '.hydro';
			...
			try
			{
				if (this.fileObject == null)
				{
					...
					var path = await requestSync({
						...
						action: 'showSaveDialog',
						defaultPath: (lastDir || (await requestSync('getDocumentsFolder'))) + '/' + name,
						filters: this.ui.createFileSystemFilters('hydro')
					});
...
App.prototype.createFileSystemFilters = function (defaultExt) {
    var ext = [
        { name: 'Hydro Calculation File (.hydro)', extensions: ['hydro'] } //,
    ...
    // 기본 확장자 우선 설정 hydro_yangs
    // if (defaultExt === 'hydro') {
    //     ext.sort((a, b) => (a.extensions[0] === 'hydro' ? -1 : 1));
    // }
	...
	EditorUi.prototype.saveData = async function(filename, format, data, mimeType, base64Encoded)
	{
		...
		window.setTimeout(mxUtils.bind(this, async function()
		{
			...
			switch (format)
			{
				...
				case 'hydro':
					filters = [
				          { name: 'hydro calculation file', extensions: ['hydro'] }
				       ];
				break;				
			};
